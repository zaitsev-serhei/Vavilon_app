...
class TransactionRepository @Inject constructor(
    ...
    suspend fun addTransactionToPlan(transaction: TransactionEntity, planId: Long) {
        withContext(Dispatchers.IO) {
            val trId = transactionDao.insert(transaction)
            transactionDao.insertTransactionForSource(transaction.sourceId, trId)
            transactionDao.insertTransactionForPlan(planId, trId)
        }
    }

    fun getCategorizedTransactionList(category: TransactionCategories): Flow<List<TransactionEntity>> {
